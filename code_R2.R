...
info_final_samples_selected_test_All <- info_final_samples_selected[c(3:6),]

rownames(info_final_samples_selected_test_All) <- c()

rownames_tests <- do.call(rbind,strsplit(info_final_samples_selected_test_All[,1], ":"))

rownames(info_final_samples_selected_test_All) <- rownames_tests[,1]
 
 
 
for(j in 1:ncol(info_final_samples_selected_test_All)){
c <- strsplit(info_final_samples_selected_test_All[,j], ":")
rej <- do.call(rbind,list(c$"rejection/non rejection"))

info_final_samples_selected_test_All [1,j] <- c$"time of biopsy post transplant (days)"[2]
info_final_samples_selected_test_All [2,j] <- c$"time from biopsy to failure/censoring (days)"[2]

info_final_samples_selected_test_All [3,j] <- rej[2]

info_final_samples_selected_test_All [4,j] <- c$"sample included in the main analysis- the first biopsy per patient from the 105 late (> 1 year) patients"[2]
 
}

info_final_samples_selected_test_All <- info_final_samples_selected_test_All[,order(info_final_samples_selected_test_All[3,])]
###### rej / nonrej#####
 index_rej <-  which( info_final_samples_selected_test_All[3,]==" rej")

 index_nonrej <-  which(info_final_samples_selected_test_All[3,]==" nonrej") 

 samples_index_rej_valid <- sample(index_rej,5)

 samples_index_nonrej_valid <- sample(index_nonrej,5)

 samples_index_validation <- c(samples_index_rej_valid,samples_index_nonrej_valid)
 
 samples_Apprentissage <- info_final_samples_selected_test_All[,-samples_index_validation]

 samples_validation <- info_final_samples_selected_test_All[,samples_index_validation]
 

 info_final_samples_selected_test <- samples_Apprentissage[c(1,2,4),]

 final_samples_selected_App <- final_samples_selected[c(1:21),-samples_index_validation]


 Samples_selected_First_20_probs <- rbind(final_samples_selected_App,info_final_samples_selected_test)
